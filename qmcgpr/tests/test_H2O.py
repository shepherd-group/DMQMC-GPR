...
class BenchmarkData:
    ...
    def __init__(self) -> None:
        ...
        self.betafit_full = numpy.load(
            'H2O/benchmark_betafit_full_domain.npy'
        )
        self.betafit_sub = numpy.load(
            'H2O/benchmark_betafit_sub_domain.npy'
        )
        self.tempfit_full = numpy.load(
            'H2O/benchmark_tempfit_full_domain.npy'
        )
        self.tempfit_sub = numpy.load(
            'H2O/benchmark_tempfit_sub_domain.npy'
        )
...
class TestRestartGeneration(unittest.TestCase, BenchmarkData, SystemData):
    ...
    def doCleanups(self):
        ...
        if os.path.isfile('H2O/betafit_full_domain.npy'):
            os.remove('H2O/betafit_full_domain.npy')
        if os.path.isfile('H2O/tempfit_full_domain.npy'):
            os.remove('H2O/tempfit_full_domain.npy')
        if os.path.isfile('H2O/betafit_sub_domain.npy'):
            os.remove('H2O/betafit_sub_domain.npy')
        if os.path.isfile('H2O/tempfit_sub_domain.npy'):
            os.remove('H2O/tempfit_sub_domain.npy')
    ...
    def test_full_betafit(self):
        ...
        GPyDMQMC(
            ...
            y,
            self.beta,
            save_file='H2O/betafit_full_domain.npy',
            kernel_product=True,
        )
        betafit_full = numpy.load('H2O/betafit_full_domain.npy')
    ...
    def test_full_tempfit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.temp[1:],
            save_file='H2O/tempfit_full_domain.npy',
            kernel_product=True,
        )
        tempfit_full = numpy.load('H2O/tempfit_full_domain.npy')
    ...
    def test_sub_betafit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.beta,
            save_file='H2O/betafit_sub_domain.npy',
            kernel_product=True,
        )
        betafit_sub = numpy.load('H2O/betafit_sub_domain.npy')
    ...
    def test_sub_tempfit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.temp[1:],
            save_file='H2O/tempfit_sub_domain.npy',
            kernel_product=True,
        )
        tempfit_sub = numpy.load('H2O/tempfit_sub_domain.npy')
