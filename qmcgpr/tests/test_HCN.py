...
class SystemData:
    ...
    def __init__(self) -> None:
        self.dmqmc_data = pandas.read_csv(
            'HCN/pyhande_dmqmc_fta.csv'
        )
        self.beta_nslice = 10
        self.beta_cutoff = 0.5
        self.temp_nslice = 10
        self.temp_cutoff = 1.0
        self.fci = -91.841001346958
...
class BenchmarkData:
    ...
    def __init__(self) -> None:
        self.data = pandas.read_csv(
            'HCN/benchmark_gpr_data.csv'
        )
        self.betafit_full = numpy.load(
            'HCN/benchmark_betafit_full_domain.npy'
        )
        self.betafit_sub = numpy.load(
            'HCN/benchmark_betafit_sub_domain.npy'
        )
        self.tempfit_full = numpy.load(
            'HCN/benchmark_tempfit_full_domain.npy'
        )
        self.tempfit_sub = numpy.load(
            'HCN/benchmark_tempfit_sub_domain.npy'
        )
...
class TestRestartGeneration(unittest.TestCase, BenchmarkData, SystemData):
    ...
    def doCleanups(self):
        ...
        if os.path.isfile('HCN/betafit_full_domain.npy'):
            os.remove('HCN/betafit_full_domain.npy')
        if os.path.isfile('HCN/tempfit_full_domain.npy'):
            os.remove('HCN/tempfit_full_domain.npy')
        if os.path.isfile('HCN/betafit_sub_domain.npy'):
            os.remove('HCN/betafit_sub_domain.npy')
        if os.path.isfile('HCN/tempfit_sub_domain.npy'):
            os.remove('HCN/tempfit_sub_domain.npy')
    ...
    def test_full_betafit(self):
        ...
        GPyDMQMC(
            ...
            y,
            self.beta,
            save_file='HCN/betafit_full_domain.npy',
        )
        betafit_full = numpy.load('HCN/betafit_full_domain.npy')
    ...
    def test_full_tempfit(self):
        ...
        GPyDMQMC(
            ...
            y,
            self.temp[1:],
            save_file='HCN/tempfit_full_domain.npy',
        )
        tempfit_full = numpy.load('HCN/tempfit_full_domain.npy')
    ...
    def test_sub_betafit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.beta,
            save_file='HCN/betafit_sub_domain.npy',
        )
        betafit_sub = numpy.load('HCN/betafit_sub_domain.npy')
    ...
    def test_sub_tempfit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.temp[1:],
            save_file='HCN/tempfit_sub_domain.npy',
        )
        tempfit_sub = numpy.load('HCN/tempfit_sub_domain.npy')
