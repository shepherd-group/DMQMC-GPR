...
class SystemData:
    ...
    def __init__(self) -> None:
        self.dmqmc_data = pandas.read_csv(
            'CO/pyhande_dmqmc_fta.csv'
        )
        self.beta_nslice = 10
        self.beta_cutoff = 0.2
        self.temp_nslice = 10
        self.temp_cutoff = 1.0
        self.fci = -111.363009273044
...
class BenchmarkData:
    ...
    def __init__(self) -> None:
        self.data = pandas.read_csv(
            'CO/benchmark_gpr_data.csv'
        )
        self.betafit_full = numpy.load(
            'CO/benchmark_betafit_full_domain.npy'
        )
        self.betafit_sub = numpy.load(
            'CO/benchmark_betafit_sub_domain.npy'
        )
        self.tempfit_full = numpy.load(
            'CO/benchmark_tempfit_full_domain.npy'
        )
        self.tempfit_sub = numpy.load(
            'CO/benchmark_tempfit_sub_domain.npy'
        )
...
class TestRestartGeneration(unittest.TestCase, BenchmarkData, SystemData):
    ...
    def doCleanups(self):
        ...
        if os.path.isfile('CO/betafit_full_domain.npy'):
            os.remove('CO/betafit_full_domain.npy')
        if os.path.isfile('CO/tempfit_full_domain.npy'):
            os.remove('CO/tempfit_full_domain.npy')
        if os.path.isfile('CO/betafit_sub_domain.npy'):
            os.remove('CO/betafit_sub_domain.npy')
        if os.path.isfile('CO/tempfit_sub_domain.npy'):
            os.remove('CO/tempfit_sub_domain.npy')
    ...
    def test_full_betafit(self):
        ...
        GPyDMQMC(
            ...
            y,
            self.beta,
            save_file='CO/betafit_full_domain.npy',
        )
        betafit_full = numpy.load('CO/betafit_full_domain.npy')
    ...
    def test_full_tempfit(self):
        ...
        GPyDMQMC(
            ...
            y,
            self.temp[1:],
            save_file='CO/tempfit_full_domain.npy',
        )
        tempfit_full = numpy.load('CO/tempfit_full_domain.npy')
    ...
    def test_sub_betafit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.beta,
            save_file='CO/betafit_sub_domain.npy',
        )
        betafit_sub = numpy.load('CO/betafit_sub_domain.npy')
    ...
    def test_sub_tempfit(self):
        ...
        GPyDMQMC(
            ...
            y[mask],
            self.temp[1:],
            save_file='CO/tempfit_sub_domain.npy',
        )
        tempfit_sub = numpy.load('CO/tempfit_sub_domain.npy')
